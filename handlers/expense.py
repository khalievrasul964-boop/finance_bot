...
async def expense_start(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            ...
            "(например: <code>300</code> или <code>299.90</code>)",
            parse_mode="HTML",
            reply_markup=get_cancel_keyboard()
        )
        await state.set_state(ExpenseFlow.waiting_for_amount)
    except Exception as e:
        logger.error(f"❌ Ошибка в expense_start: {e}", exc_info=True)
        await message.answer("❌ Произошла ошибка. Попробуйте позже.")

@router.message(ExpenseFlow.waiting_for_amount)
async def expense_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        await state.set_state(ExpenseFlow.waiting_for_category)
    except Exception as e:
        logger.error(f"❌ Ошибка в expense_amount: {e}", exc_info=True)
        await message.answer("❌ Произошла ошибка. Попробуйте еще раз.")

@router.callback_query(ExpenseFlow.waiting_for_category)
async def expense_category(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        category = EXPENSE_CATEGORIES[category_idx]
        ...
        await state.set_state(ExpenseFlow.waiting_for_method)
        await callback.answer()
    except Exception as e:
        logger.error(f"❌ Ошибка в expense_category: {e}", exc_info=True)
        await callback.answer("❌ Произошла ошибка", show_alert=True)

@router.callback_query(ExpenseFlow.waiting_for_method)
async def expense_method(callback: CallbackQuery, state: FSMContext):
    """Обработка способа оплаты расхода (callback)."""
    try:
        ...
        if success:
            ...
            await callback.message.answer(
                "❌ Ошибка при добавлении расхода. Попробуйте позже.",
                reply_markup=get_main_menu()
            )
        ...
        logger.error(f"❌ Ошибка в expense_method: {e}", exc_info=True)
        await callback.answer("❌ Произошла ошибка", show_alert=True)
        await state.clear()